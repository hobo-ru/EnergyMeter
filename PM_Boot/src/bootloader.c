...
#error "Can not make correct clock selection."
  ...
  if(GPIO_PinInGet( BUTTON1.port, BUTTON1.pin ) == 0 )      //при двух нажатых кнопках прошиваем "родную" прошивку
  {
  ...
  FLASH_ReadBuffer(update_address  , (uint8_t*)&s_up, sizeof(s_up)); //чтение структуры описывающей обновление 
  if(CRC_calc((void *)s_up.start_add,(void *)s_up.end_add) == s_up.crc) BOOT_boot();  //запуск приложения, если обновление совпадает с приложением
  
  if(FLASH_CRC_calc(update_address + s_up.start_add, update_address + s_up.end_add) != s_up.crc)  BOOT_boot();// у обновления не сходится CRC
