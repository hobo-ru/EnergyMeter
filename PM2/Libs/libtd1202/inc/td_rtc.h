...
extern void TD_RTC_AddOffsetDelta(int delta);
extern void TD_RTC_SetOffsetTime(time_t offsetTime);
extern uint32_t NOW;
 ...
 *   Set user RTC time.
 ******************************************************************************/
static __INLINE void TD_RTC_SetTime(uint32_t time)
{
  int32_t delta = time - __time32(0);
  if( delta != 0) {
    TD_RTC_AddOffsetDelta( delta );
  }
